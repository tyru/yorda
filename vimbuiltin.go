...
var vimFunctions = []vimFunc{
	...
	{"add", func() vimType {
		// TODO generics
		return newFuncType(newTupleType(typeList, typeAny), typeList)
	}},
	{"and", func() vimType {
	...
	{"balloon_show", func() vimType {
		return newFuncType(newTupleType(
			newUnionType(typeString, &applyType{typeList, typeString}),
		), typeInt)
	}},
	{"balloon_split", func() vimType {
		return newFuncType(newTupleType(typeString), &applyType{typeList, typeString})
	}},
	{"browse", func() vimType {
...
func lookUpVar(scope, name string) *vimVar {
	...
	i := sort.Search(len(vimVariables), func(i int) bool {
		n := vimVariables[i].scope + ":" + vimVariables[i].scope
